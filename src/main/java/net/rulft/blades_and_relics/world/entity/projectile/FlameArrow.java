...
public class FlameArrow extends AbstractArrow {
    ...
    public FlameArrow(EntityType<? extends AbstractArrow> type, Level level) {
        super(type, level);
        this.referenceItem = ModItems.FLAME_ARROW.get();
    }

    public FlameArrow(LivingEntity shooter, Level level, Item referenceItem) {
        super(ModEntityType.FLAME_ARROW.get(), shooter, level);
    ...
    public void tick() {
        ...
        if (this.level.isClientSide && !this.inGround) {
            ...
            IceAndFire.PROXY.spawnParticle(EnumParticles.DragonFire, this.getX() + xRatio + (double)(this.random.nextFloat() * this.getBbWidth() * 1.0F) - (double)this.getBbWidth() - d0 * 10.0, this.getY() + (double)(this.random.nextFloat() * this.getBbHeight()) - d1 * 10.0, this.getZ() + zRatio + (double)(this.random.nextFloat() * this.getBbWidth() * 1.0F) - (double)this.getBbWidth() - d2 * 10.0, 0.1, 1.0, 0.1);
            IceAndFire.PROXY.spawnParticle(EnumParticles.DragonFire, this.getX() + xRatio + (double)(this.random.nextFloat() * this.getBbWidth() * 1.0F) - (double)this.getBbWidth() - d0 * 10.0, this.getY() + (double)(this.random.nextFloat() * this.getBbHeight()) - d1 * 10.0, this.getZ() + zRatio + (double)(this.random.nextFloat() * this.getBbWidth() * 1.0F) - (double)this.getBbWidth() - d2 * 10.0, 0.1, 1.0, 0.1);
        }
